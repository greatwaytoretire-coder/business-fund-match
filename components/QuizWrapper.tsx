...
export default function QuizWrapper() {
  ...
  const handleSelectOption = (key: string, value: string) => {
    updateQuizState({ [key]: value, currentStep: currentStep + 1 });
  };

  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    updateQuizState({ [e.target.name]: e.target.value });
  };

  const handleNext = () => {
    if (currentStep < steps.length - 1) {
      updateQuizState({ currentStep: currentStep + 1 });
    }
  };
  ...
  const handleSubmitFinalLead = async (e: React.FormEvent) => {
    ...
    // 🚀 LIVE TESTING SIMULATION: Form Payload Preparation
    const leadPayload = {
      amount: quizState.fundingAmount,
      tenure: quizState.timeInBusiness,
      revenue: quizState.monthlyRevenue,
      credit: quizState.creditScore,
      businessName: quizState.businessName,
    ...
    console.log("🔥 LEAD FORM SUBMITTED! FULL DATA PAYLOAD BELOW:", leadPayload);

    // Simulate network lag to test loading state indicators
    setTimeout(() => {
      setLoading(false);
      setIsSubmitted(true);
      clearQuizState(); // Clear browser memory only AFTER a successful layout commit
    }, 1500);
  };

  if (isSubmitted) {
    return (
      <div className="bg-white rounded-2xl border-2 border-green-500/20 p-8 text-center space-y-6 shadow-xl max-w-xl mx-auto my-8 transform animate-fadeIn">
        ...
        <div className="space-y-2">
          <h3 className="text-2xl font-black tracking-tight text-slate-900">Pre-Qualification Complete!</h3>
          <p className="text-sm text-slate-500 max-w-md mx-auto font-medium">
            Your funding criteria profile matches our elite-tier algorithmic parameters. A capital execution specialist will patch into your line shortly.
          </p>
        </div>
      </div>
    );
      ...
      <div className="p-6 sm:p-8 min-h-[320px] flex flex-col justify-between">
        ...
        <div className="flex items-center justify-between border-t border-slate-100 pt-4 mt-6">
          ...
          {currentStep === steps.length - 1 ? (
            <button type="submit" form="leadCaptureForm" disabled={loading} className="bg-blue-600 hover:bg-blue-700 text-white font-black text-xs px-6 py-3 rounded-full shadow-md transition uppercase tracking-wider flex items-center gap-2 disabled:bg-blue-400">
              {loading ? 'Routing Application...' : 'Secure Pre-Qualify Match'} <ChevronRight size={14} />
            </button>
          ) : (
            <button onClick={handleNext} disabled={currentStep === steps.length - 1} className="flex items-center gap-1 text-xs font-black text-blue-600 hover:text-blue-700 transition uppercase tracking-wider">
